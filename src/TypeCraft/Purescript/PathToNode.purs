...
termPathToNode isActive abovePath belowInfo termPath innerNode =
  ...
  in
    recTermPath
      ...
      , buffer1:
          \upRecVal md {-Term-} bufTy body bodyTy ->
            let
              newBI = stepBI (Buffer1 md {--} bufTy.ty body.term bodyTy) belowInfo
            in
              termPathToNode isActive abovePath newBI upRecVal
                $ arrangeTerm (makeTermArgs isActive abovePath newBI upRecVal)
                    [ arrangeKid upRecVal.termPath abovePath (\_ _ -> innerNode) term
      ...
      , buffer3:
          \upRecVal md buf bufTy {-Term-} bodyTy ->
            let
              newBI = stepBI (Buffer3 md buf.term bufTy.ty {--} bodyTy) belowInfo
            in
              termPathToNode isActive abovePath newBI upRecVal
                $ arrangeTerm (makeTermArgs isActive abovePath newBI upRecVal)
      ...
      , typeBoundary1:
          \upRecVal md ch {-Term-} ->
            let
              newBI = stepBI (TypeBoundary1 md ch {--}) belowInfo
            in
              termPathToNode isActive abovePath newBI upRecVal
                $ arrangeTerm (makeTermArgs isActive abovePath newBI upRecVal)
                    [ arrangeKid upRecVal.termPath abovePath (\_ _ -> innerNode) term
      ...
      , contextBoundary1:
          \upRecVal md x change {-Term-} ->
            let
              newBI = stepBI (ContextBoundary1 md x change {--}) belowInfo
            in
              termPathToNode isActive abovePath newBI upRecVal
                $ arrangeTerm (makeTermArgs isActive abovePath newBI upRecVal)
                    [ arrangeKid upRecVal.termPath abovePath (\_ _ -> innerNode) term
  ...
  in
    recTypePath
      ( { lambda2:
        ...
        , buffer2:
            \termPath md def {-Type-} body bodyTy ->
              let
                newBI = BITerm
              in
                termPathToNode isActive abovePath newBI termPath
        ...
        , ctrParam1:
            \ctrParamPath md {-Type-} ->
              let
                newBI = BITerm
              in -- TODO: This is one place where I have to go back and change things to make selection in type arguments work!
...
ctrParamPathToNode isActive abovePath belowInfo ctrParamPath innerNode =
  let
    ctrParam = ctrParamPath.ctrParam
  in
    recCtrParamPath
      { ctrParamListCons1:
          \listCtrParamPath {-ctrParam-} ctrParams ->
            let
              newBI = BITerm
            in
...
typeArgPathToNode isActive abovePath belowInfo typeArgPath innerNode =
  let
    tyArg = typeArgPath.tyArg
  in
    recTypeArgPath
      { typeArgListCons1:
          \listTypeArgPath {-tyArg-} tyArg ->
            let
              newBI = BITerm
            in
...
typeBindPathToNode isActive abovePath typeBindPath innerNode =
  ...
  in
    recTypeBindPath
      ...
      , typeBindListCons1:
          \listTypeBindPath {-tyBind-} tyBinds ->
            let
              newBI = BITerm
            in
  ...
  in
    recListCtrPath
      ...
      , ctrListCons2:
          \listCtrPath ctr {-ctrs-} ->
            let
              newBI = stepBI (CtrListCons2 ctr.ctr) belowInfo
            in
              ctrListPathToNode isActive abovePath newBI listCtrPath
                $ arrangeCtrList (makeCtrListArgs isActive abovePath newBI listCtrPath)
...
ctrParamListPathToNode isActive abovePath belowInfo listCtrParamPath innerNode =
  let
    ctrParams = listCtrParamPath.ctrParams
  in
    recListCtrParamPath
      { constructor2:
          \ctrPath md tBind {-ctrParams-} ->
      ...
      , ctrParamListCons2:
          \listCtrParamPath ctrParam {-ctrParams-} ->
            let
              newBI = stepBI (CtrParamListCons2 ctrParam.ctrParam {-ctrParams-}) belowInfo
            in
              ctrParamListPathToNode isActive abovePath newBI listCtrParamPath
                $ arrangeCtrParamList (makeCtrParamListArgs isActive abovePath newBI listCtrParamPath)
...
typeArgListPathToNode isActive abovePath belowInfo listTypeArgPath innerNode =
  ...
  in
    recListTypeArgPath
      ...
      , typeArgListCons2:
          \listTypeArgPath tyArg ->
            let
              newBI = stepBI (TypeArgListCons2 tyArg.tyArg {-tyArgs-}) belowInfo
            in
              typeArgListPathToNode isActive abovePath newBI listTypeArgPath
                $ arrangeTypeArgList (makeTypeArgListArgs isActive abovePath newBI listTypeArgPath)
    ...
    recListTypeBindPath
      ( { data2:
        ...
        , typeBindListCons2:
            \listTypeBindPath tyBind {-tyBinds-} ->
              let
                newBI = stepBI (TypeBindListCons2 tyBind.tyBind {--}) belowInfo
              in
  ...
  recInsideHolePath
    { hole1:
        \md termPath ->
          ...
          in
            termPathToNode isActive abovePath newBI termPath
              $ setNodeMetadata (makeHoleMetadata (typeToNode false AINothing { ctxs: insideHolePath.ctxs, ty: insideHolePath.ty }))
              $ arrangeTerm (makeTermArgs isActive abovePath newBI termPath)
